...
The date defaults to today in the application timezone. Arrival and departure lists include confirmed, checked-in, and completed stays for the requested date. Pending and cancelled reservations are excluded. The in-house list is evaluated at request time and includes only stays that have been checked in, have not been checked out, and contain the current date under half-open stay semantics (`check_in <= today < check_out`).
